...
<div class="min-h-screen bg-white px-6 py-8 flex flex-col pt-safe">
    ...
    @if (session('success'))
        <div class="bg-green-50 text-green-600 px-4 py-3 rounded-2xl text-sm font-bold text-center mb-6 border border-green-100 shadow-sm">
            {{ session('success') }}
        </div>
    @endif

    <form action="{{ route('login') }}" method="POST" class="space-y-6 flex-1">
        @csrf
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input name="login" type="text" value="{{ old('login') }}" class="w-full bg-slate-50 border-2 border-transparent focus:border-sky-500 rounded-2xl pr-11 pl-4 py-3.5 text-slate-900 placeholder-slate-400 transition-all font-medium focus:bg-white focus:shadow-lg focus:shadow-sky-100/50 outline-none" placeholder="example@mail.com">
            </div>
            @error('login') <span class="text-red-500 text-xs px-2 block mt-1 font-bold">{{ $message }}</span> @enderror
        </div>
    ...
    <div class="mt-8 text-center text-sm text-slate-500 font-medium">
        ليس لديك حساب؟ <a href="/register" class="text-sky-500 font-bold hover:underline">أنشئ حساباً جديداً</a>
    </div>
</div>
